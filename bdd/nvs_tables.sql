...
CREATE TABLE `anim_capture` (
 ...
 champs pour valider les 12h d encerclement
ALTER TABLE `anim_capture`
ADD `date_debut_encerclement` DATETIME NULL DEFAULT NULL COMMENT 'Date à laquelle les 8 cases ont été occupées',
ADD `dernier_check_encerclement` DATETIME NULL DEFAULT NULL COMMENT 'Dernière vérification automatique',
ADD `statut_capture` TINYINT(1) DEFAULT 0 COMMENT '0: En cours, 1: Validé, 2: Terminé/Téléporté';
ADD `clan_capture` INT(1) NOT NULL
COMMENT 'ID du clan qui réalise l\'encerclement (1 ou 2)'
AFTER `id_perso_capture`;

Exécute ce code dans l'onglet SQL de ton phpMyAdmin :
SQL
...
ALTER TABLE `compagnies`
  ...
  ALTER TABLE compagnies
ADD couleur_compagnie VARCHAR(7) DEFAULT '#ffffff'
AFTER nom_compagnie;
  --
...
ALTER TABLE `instance_batiment`
  ...
  ALTER TABLE 'instance_batiment'
  ADD COLUMN stock_actuel INT DEFAULT 0,
  ADD COLUMN dernier_update_champ DATETIME DEFAULT CURRENT_TIMESTAMP;
...
ALTER TABLE `perso`
  ...
  --
  -- Compteurs pour la recup progresive des perso sur un periode totale donnée qui stocke les poussieres de recup avant de donner dans la caracteristique l entier qui va bien
  --
...
ALTER TABLE `type_unite`
ADD `type_combat` TINYTEXT NOT NULL AFTER `nom_unite`;
-- --------------------------------------------------------
